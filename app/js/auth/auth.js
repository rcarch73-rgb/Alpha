(() => {
  ...
  let session=null, cloudPlanId=null, syncTimer=null, suppressSync=false;
  ...
  function updateUI(){
    ...
    if(el('cloudAccountCopy'))el('cloudAccountCopy').textContent=user?`Signed in as ${email}. Your plan is protected by row-level security.`:'Sign in to securely sync this plan across devices.';
    if(el('cloudAccountBtn')){el('cloudAccountBtn').classList.toggle('hidden',!!user);el('cloudAccountBtn').textContent='Sign in'}
    el('cloudSyncBtn')?.classList.toggle('hidden',!user);
  ...
  async function fetchCloudPlan(){
    ...
    const {data,error}=await client.from('plans').select('id,plan_data,updated_at').eq('user_id',session.user.id).order('updated_at',{ascending:false}).limit(1).maybeSingle();
    if(error)throw error;
    cloudPlanId=data?.id||null;
    return data;
  }
  async function saveCloudPlan(showToast=false){
    ...
    const payload={user_id:session.user.id,plan_name:plan.name1?`${plan.name1}'s retirement plan`:'My retirement plan',plan_data:plan,updated_at:new Date().toISOString()};
    let response;
    if(cloudPlanId)response=await client.from('plans').update(payload).eq('id',cloudPlanId).eq('user_id',session.user.id).select('id').single();
    else response=await client.from('plans').insert(payload).select('id').single();
    if(response.error)throw response.error;
    cloudPlanId=response.data.id;
  ...
  async function handleSignedIn(){
    ...
    const local=window.HNCloudBridge?.getPlan?.();
    if(cloud?.plan_data){
      const cloudTime=Date.parse(cloud.updated_at||0), localTime=Date.parse(local?.updatedAt||0);
      if(!local?.updatedAt||cloudTime>localTime){suppressSync=true;window.HNCloudBridge.applyPlan(cloud.plan_data);suppressSync=false;window.toast?.('Cloud plan loaded')}
      else await saveCloudPlan(false);
    }else if(local)await saveCloudPlan(false);
  }
  async function createAccount(){
    ...
    if(password.length<8)return window.toast?.('Use at least 8 characters for your password');
    if(password!==confirm)return window.toast?.('Passwords do not match');
    const button=el('createAccountBtn');setBusy(button,true,'Creating account…');
    try{
      const {data,error}=await client.auth.signUp({email,password,options:{data:{first_name:first,last_name:last}}});if(error)throw error;
      ...
      if(session){await handleSignedIn();window.go('household');window.toast?.('Account created and cloud sync enabled')}
  ...
  async function signIn(){
    const email=el('signInEmail').value.trim(),password=el('signInPassword').value,button=el('signInBtn');
    if(!email||!password)return setMessage('Enter your email and password.','error');
    setBusy(button,true,'Signing in…');setMessage('');
    try{const {data,error}=await client.auth.signInWithPassword({email,password});if(error)throw error;session=data.session;await handleSignedIn();window.go(localStorage.getItem('harbourNorth.foundation.v0.1')?'brief':'household');window.toast?.('Signed in successfully')}
    catch(e){setMessage(e.message||'Sign in failed.','error')}finally{setBusy(button,false)}
  }
  async function resetPassword(){
    const email=el('signInEmail').value.trim();if(!email)return setMessage('Enter your email address first.','error');
    try{const {error}=await client.auth.resetPasswordForEmail(email,{redirectTo:location.origin+location.pathname});if(error)throw error;setMessage('Password reset email sent.','success')}catch(e){setMessage(e.message||'Unable to send reset email.','error')}
  }
  async function signOut(){try{await client.auth.signOut();session=null;cloudPlanId=null;updateUI();window.go('welcome');window.toast?.('Signed out')}catch(e){window.toast?.(e.message||'Sign out failed')}}
  async function init(){
