(() => {
  ...
  async function createPlan(){
    ...
    window.HNAuth.setActivePlanId(data.id);window.HNAuth.setSuppressSync(true);try{window.HNCloudBridge.applyPlan(plan);window.go('household');window.toast?.('New plan created')}finally{window.HNAuth.setSuppressSync(false)};
  }
  async function openPlan(id){
    ...
    const plan={...(data.plan_data||{}),planName:data.plan_name||data.plan_data?.planName};window.HNAuth.setActivePlanId(id);window.HNAuth.setSuppressSync(true);try{window.HNCloudBridge.applyPlan(plan);window.go('brief');window.toast?.('Plan opened')}finally{window.HNAuth.setSuppressSync(false)};
  }
  ...
  async function duplicatePlan(id){const plans=await listPlans(),found=plans.find(p=>p.id===id);if(!found)return;const name=(prompt('Name the duplicate',`${found.plan_name||'Retirement plan'} copy`)||'').trim();if(!name)return;const copy={...(structuredClone(found.plan_data||{})),planName:name,updatedAt:new Date().toISOString()};const {data,error}=await client().from('plans').insert({user_id:session().user.id,plan_name:name,plan_data:copy,updated_at:new Date().toISOString()}).select('id').single();if(error)throw error;window.HNAuth.setActivePlanId(data.id);window.HNAuth.setSuppressSync(true);try{window.HNCloudBridge.applyPlan(copy);window.go('brief');window.toast?.('Plan duplicated')}finally{window.HNAuth.setSuppressSync(false)}}
