(function(){
  ...
  function getRecommendations(plan,result){
    ...
    return window.HNRecommendationEngine.rank(plan||{},result||{},3);
  }

  function planState(plan,result){
    const safePlan=plan||{};
    const safeResult=result||{};
    const ratio=num(safeResult.ratio);
    const target=num(safePlan.spend);
    const sustainable=num(safeResult.sustainable);
  ...
  function renderSummary(items,plan,result){
    const summary=ensureSummary();
    if(!summary)return;
    const state=planState(plan,result);
    const first=items[0];
  ...
  function opportunityLabel(item){
    return String(item?.category||'Planning opportunity');
  }

  function confidenceLabel(item){
    return String(item?.confidence||'Planning estimate');
  }

  function timingLabel(item){
    return String(item?.timing||'Review with your next plan update');
  }
      ...
      <div class="next-step-details">
        <div class="next-step-detail-grid">
          ...
          <div class="next-step-meta">
            <div><span>Opportunity</span><strong>${escapeHtml(opportunityLabel(item))}</strong></div>
            <div><span>Confidence</span><strong>${escapeHtml(confidenceLabel(item))}</strong></div>
            <div><span>Difficulty</span><strong>${escapeHtml(difficulty(item))}</strong></div>
            <div><span>When to review</span><strong>${escapeHtml(timingLabel(item))}</strong></div>
          </div>
        </div>
      </div>
    </article>`;
