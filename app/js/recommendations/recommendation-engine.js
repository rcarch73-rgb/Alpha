(function(){
  ...
  function safeCalculate(plan){
    try{
      const calculate=window.HNVerifiedEngine&&window.HNVerifiedEngine.calculate;
  ...
  function comparePlan(basePlan,baseResult,patch,label){
    ...
    const alternative=safeCalculate(alternativePlan);
  ...
  function buildCandidates(plan,result){
    ...
    if(ratio>0&&ratio<1){
      const oneYearLater=comparePlan(plan,result,{retire1:num(plan.retire1)+1,retire2:plan.household==='couple'?num(plan.retire2)+1:num(plan.retire2)},'Retire one year later');
      const lowerSpend=Math.max(1000,Math.round(num(plan.spend)*.95/100)*100);
      const spendingComparison=comparePlan(plan,result,{spend:lowerSpend},`Reduce spending to ${money(lowerSpend)}/month`);
    ...
    if(annualSpend>0&&ratio>=1.15){
      ...
      const comparison=comparePlan(plan,result,{spend:higherSpend},`Test ${money(higherSpend)}/month spending`);
      items.push(candidate({
